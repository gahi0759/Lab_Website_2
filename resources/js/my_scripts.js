...
function loadStatsPage(){
	...
	for(row_counter = 2; row_counter < table.rows.length; row_counter++){
		...
		opp_score = table.rows[row_counter].cells[3].innerHTML;
		if (home_value > opp_score){
			table.rows[row_counter].cells[4].innerHTML = "CU Boulder";
			wins++;
		}
		else{
			opp_name = table.rows[row_counter].cells[1].innerHTML;
...
function loadPlayersPage(){
	...
	for (var i = 0; i < players.length; i++){
		x = x + "<a href='#' class='dropdown-item' onclick='switchPlayers("+i+")'>"+players[i].name+"</a>";
	}
	document.getElementById('player_selector').innerHTML = x;
}

function switchPlayers(playerNum){
	var g = players[playerNum].games_player;
	var hold;
	...
	document.getElementById("g_played").innerHTML = g;
	...
	hold = players[playerNum].pass_yards/g;
	hold = hold.toFixed(0);
	document.getElementById("avg_yards").innerHTML = hold;
	hold = players[playerNum].rushing_yards/g;
	hold = hold.toFixed(0);
	document.getElementById("avg_r_yards").innerHTML = hold;
	hold = players[playerNum].receiving_yards/g;
	hold = hold.toFixed(0);
	document.getElementById("avg_rec_yards").innerHTML = hold;
}
